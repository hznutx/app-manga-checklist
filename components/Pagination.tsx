...
const PageLayout = ({ WebtoonCard }: { WebtoonCard: any }) => {
      ...
      <div className="w-full flex rounded-3xl">
        {activeTab === Tab.grid ? (
          <div
            ...
            role="tab"
            className="px-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 w-full gap-12 items-center"
          >
            {WebtoonCard.map((movie: any, i: any) => (
              <WebtoonCard movie={movie} key={i} />
            ))}
          </div>
        ) : activeTab === Tab.card ? (
